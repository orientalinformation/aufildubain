...
@foreach ($items as $item)
    @php
        ...
        if (empty($options->active_slug)) 
            $active_slug = '/admin';
        else
            $active_slug = '/admin/'.$options->active_slug;

        // Current page
        if($href == $active_slug) 
            array_push($listItemClass, 'active');
        elseif (url($href) == url()->current())
            array_push($listItemClass, 'active');
        ...
        if(!$item->children->isEmpty())
        {
            foreach($item->children as $child)
            {
                ...
                $childHref = $child->link();

                if($childHref == $active_slug)
                    array_push($listItemClass, 'active');
                elseif (url($childHref) == url()->current())
                    array_push($listItemClass, 'active');
