                                            ...
                                            <div class="right_details">
                                                ...
                                                <div class="detail_info">
                                                    <div :class="selected_file.type">
                                                        ...
                                                        <span ><h4>Type:</h4>
                                                            <p class="selected_type">@{{selected_file.type}}</p>
                                                        </span>
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                <div class="crop-container">
                                                    <img v-if="selectedFileIs('image')" id="cropping-image" class="img img-fluid" :src="selected_file.path + '?' + selected_file.last_modified"/>
                                                </div>
...
<script>
    ...
    window.doSelect = function() {
        var type = $('.detail_info .selected_type').text().trim();

        if (type.startsWith('image/')) {
            
            var fullPath = $('.detail_info a').attr('href');
            var path = $('.detail_info a').attr('href').replace(/^(?:https?:\/\/)?(?:www\.)?/i, "")
                            .split('/storage/').slice(1).join('/');
       
            window.parent.mediaBrowser.select(path, fullPath);
        } else {
            let alerts = {
                type: 'error',
                message: '{{ __('Only image file allowed!') }}'
            }
            helpers.displayAlert(alerts, toastr);
        }
    }

    window.doSelectTrend = function() {
        var type = $('.detail_info .selected_type').text().trim();
        ...
        if (type.startsWith('image/')) {
            ...
            window.parent.mediaTrend.select(path, fullPath);
        } else {
            let alerts = {
                type: 'error',
                message: '{{ __('Only image file allowed!') }}'
